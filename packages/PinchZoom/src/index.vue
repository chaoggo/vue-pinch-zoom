...
export default class PinchZoom extends Vue {
  ...
  myEventHandler(event:any) {
      ...
      const scale = event.detail.scale && event.detail.scale > 1 ? event.detail.scale : 1;
      const moveX = event.detail.moveX || 0;
      const moveY = event.detail.moveY || 0;
      const toEl = event.detail.event.target.localName || null;
